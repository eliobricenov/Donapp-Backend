...
export const requestQueries = { 
    fetch: 'SELECT pk_post_request as id, fk_target_user as "targetUser", p.person_fk as "sourceId", p.post_pk as "postId", rs.name as status FROM person_post_request JOIN post p on person_post_request.fk_post = p.post_pk JOIN request_status rs on person_post_request.fk_request_status = rs.pk_request_status  WHERE post_type_fk = ${type} ORDER BY pk_post_request DESC FETCH FIRST ${size} ROWS ONLY; ',
